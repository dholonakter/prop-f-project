...
namespace ThanhDLL
{
    public class CrossThreadDisplay : IDisposable
    {
        ...
        delegate void DisableDelegate(Control c);
        ...
        // Set components to disabled
        public void ToggleVisibility(Control c)
        {
            if (((Control)c).InvokeRequired)
            {
                DisableDelegate d = new DisableDelegate(ToggleVisibility);
                myForm.Invoke(d, new object[] { c });
            }
            else
            {
                c.Visible = !c.Visible;
